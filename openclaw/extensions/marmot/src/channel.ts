...
function buildAvatarPrompt(avatar: AvatarConfig): string {
  ...
  parts.push(
    ...
    "Keep the scene simple: neutral background (#1a1a2e), basic lighting. IMPORTANT: VRM models face -Z, so rotate the model with vrm.scene.rotation.y = Math.PI to face the camera. After loading, compute the bounding box with new THREE.Box3().setFromObject(vrm.scene), get the center and size. Frame the upper body: var headY = box.min.y + size.y * 0.85; camera.position.set(0, headY, size.y * 0.9); camera.lookAt(0, headY - size.y * 0.05, 0); Add a subtle idle breathing animation: in the animation loop, use the humanoid bone (var chest = vrm.humanoid.getBoneNode(THREE_VRM.VRMSchema.HumanoidBoneName.Chest)) and gently oscillate chest.rotation.x with Math.sin(time) * 0.02 for a breathing effect. Also add a very slight head sway on the Head bone.",
  );
